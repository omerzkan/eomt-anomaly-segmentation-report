\section{Results}
Semantic, and anomaly segmentation evaluated on Cityscapes val and five benchmarks sequentially.
...
\centering
\small
\caption{Per-class IoU (\%) on \texttt{Cityscapes val}: EoMT-COCO (zero-shot with class mapping) 
vs EoMT-Cityscapes.}
\label{tab:perclass}
\begin{tabular}{lcc}
\toprule
...
EoMT-COCO scores 0\% IoU on \texttt{pole, rider}, and near-0\% on \texttt{traffic sign}~\ref{tab:perclass} because COCO has no equivalent label.
...
Per-model results are reported in here~\ref{tab:temperature_combined}.
